...
@section('head-tag')
    <title>دسته بندی تیکت - {{ $category->name }}</title>
@endsection
            ...
            <section class="col-12">
                <section class="main-body-container">
                    <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                        <a href="{{ route('admin.ticket-category.index') }}" class="btn btn-outline-info btn-sm">بازگشت</a>
                    </section>
                    <section class="card border-0 rounded-4">
                        <section class="card-body">
                            <form id="form" action="{{ route('admin.ticket-category.update', $category->id) }}"
                                method="POST">
                                @csrf
                                {{ method_field('put') }}
                                <section class="row">
...
@section('script')
    {!! JsValidator::formRequest('Modules\Ticket\Http\Requests\TicketCategory\TicketCategoryUpdate') !!}
    <script>
